...
def train(args, train_loader, model, criterion, optimizer, writer):
    ...
    for step, ((x_i, x_j), y) in enumerate(train_loader):
        ...
        if y.sum() == 0:
            ...
            # constrain (optional)
            Ng_pos = torch.clamp(Ng_pos, min = 0)
            # contrastive loss
            loss_pos = (- torch.log(aug_pos / (aug_pos + Ng_pos) )).mean()

            # unlabeled class augmentations
            Ng_unl_pos = (-args.tau_plus * n_unl * sim_pos.mean(dim=1) + sim_unl_inv.sum(dim=1)) / (1 - args.tau_plus)
            ...
            loss_unl = (- torch.log(aug_unl / (anchor_unl + args.tau_plus*Ng_unl_pos + (1-args.tau_plus)*Ng_unl_neg) )).mean()

            prior_prime = 0.5

            loss = prior_prime * loss_pos + (1-prior_prime) * loss_unl
