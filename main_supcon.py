...
def get_negative_mask(batch_size):
    negative_mask = torch.ones((2 * batch_size, 2 * batch_size), dtype=bool)
    ...
    for i in range(batch_size):
        for j in range(batch_size):
            for lab in torch.unique(labels):
                if (labels[i] == lab) and (labels[j] == lab):
                    mat[i, j] = lab
                    mat[i, j + batch_size] = lab
...
def train(args, train_loader, model, criterion, optimizer, writer):
    ...
    for step, ((x_i, x_j), y) in enumerate(train_loader):
        ...
        if y.sum() == 0:
            ...
            anchor = exp_sim.masked_select(mask_sample).view(2 * args.batch_size, -1).sum(dim=1).unsqueeze(1)
